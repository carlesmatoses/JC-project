...
class Scene {
	...
	collisions() {
		...
		this.levelContent.forEach((element) => {
			if (this.player.collidesWith(element)) {
				this.handleCollision(this.player, element);
			}
		});
	...
	levelTransition(from, to, deltaTime) {
	...
	handleCollision(player, element)
	{
		if (element.type==="door") {
			if (!element.isActive()) return; // Skip if the door is not active
			if (player.collidesWith(element)) {
				player.setPosition(element.door.x, element.door.y); // place user on new door position 
